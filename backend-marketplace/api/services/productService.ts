...
export class ProductService {
  ...
  static async getRelatedProducts(category: string, excludeId: string, limit: number = 5) {
    const whereCondition = and(eq(products.category, category), ne(products.id, excludeId));

    return await db.select().from(products).where(whereCondition).limit(limit).execute();
  }
}
