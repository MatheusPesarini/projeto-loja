...
export class ProductController {
  ...
  static async getProductsByCategory(request: FastifyRequest, reply: FastifyReply) {
    ...
    if (!paramsValidation.success) {
      return reply
        .status(400)
        .send(errorResponse('Parâmetros inválidos', paramsValidation.error.flatten()));
    }
    ...
    try {
      ...
      if (products.length > 0) {
        reply
          .status(200)
          .send(successResponse(products, `Produtos encontrados na categoria: ${category}`));
      } else {
  ...
  static async getProductById(request: FastifyRequest, reply: FastifyReply) {
    ...
    if (!paramsValidation.success) {
      return reply
        .status(400)
        .send(errorResponse('ID do produto inválido', paramsValidation.error.flatten()));
    }
  ...
  static async getRelatedProducts(request: FastifyRequest, reply: FastifyReply) {
    ...
    if (!queryValidation.success) {
      ...
      return reply
        .status(400)
        .send(errorResponse('Query parameters inválidos', queryValidation.error.flatten()));
    }
  ...
  static async searchProducts(request: FastifyRequest, reply: FastifyReply) {
    ...
    if (!paramsValidation.success) {
      return reply
        .status(400)
        .send(errorResponse('Termo de busca inválido', paramsValidation.error.flatten()));
    }
