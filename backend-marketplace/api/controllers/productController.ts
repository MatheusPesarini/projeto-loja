...
export class ProductController {
			...
			if (products.length > 0) {
				reply
					...
					.send(
						successResponse(
							products,
							`Produtos encontrados na categoria: ${category}`,
						),
					);
			} else {
				reply
	...
	static async getProductById(request: FastifyRequest, reply: FastifyReply) {
		...
		try {
			...
			reply
				.status(500)
				.send(errorResponse('Erro interno ao buscar produto por ID'));
		}
	}
	...
	static async getRelatedProducts(
		...
		const paramsValidation = relatedProductsParamsSchema.safeParse(
			request.params,
		);
		...
		try {
			...
			if (products.length > 0) {
				reply.send(
					successResponse(products, 'Produtos relacionados encontrados'),
				);
			} else {
				reply
					.status(404)
					.send(errorResponse('Nenhum produto relacionado encontrado'));
			}
		} catch (error) {
			request.log.error(
				error,
				`Erro ao buscar produtos relacionados para a categoria: ${category}`,
			);
			reply
				.status(500)
				.send(errorResponse('Erro interno ao buscar produtos relacionados'));
		}
	}
	...
	static async searchProducts(request: FastifyRequest, reply: FastifyReply) {
		...
		try {
			...
			request.log.error(
				error,
				`Erro ao buscar produtos com termo: ${searchTerm}`,
			);
