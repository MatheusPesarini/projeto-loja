...
export class AuthController {
	static async registerUser(request: FastifyRequest, reply: FastifyReply) {
		...
		try {
			...
			reply
				.status(201)
				.send(successResponse(newUser, 'Cadastro realizado com sucesso'));
		} catch (error) {
			request.log.error(error, 'Erro ao registrar usuário');
			reply
				.status(500)
				.send(errorResponse('Erro interno ao registrar usuário'));
		}
	}
	...
	static async loginUser(request: FastifyRequest, reply: FastifyReply) {
		...
		try {
			...
			request.log.info(
				`Login realizado com sucesso para o usuário: ${user.id}`,
			);
			...
			if (error instanceof Error && error.message.includes('JWT_SECRET_KEY')) {
				return reply
					.status(500)
					.send(errorResponse('Erro de configuração interna do servidor'));
			}
	...
	static async deleteUser(request: FastifyRequest, reply: FastifyReply) {
		...
		try {
			...
			return reply
				.status(500)
				.send(errorResponse('Erro interno ao verificar sessão'));
		}
